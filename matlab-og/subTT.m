...
for i = 1:d
    ...
    % index
    A.cores{i,1} = reshape(A.cores{i,1},[a(1),a(2)*a(3)]);
    B.cores{i,1} = reshape(B.cores{i,1},[b(1),b(2)*b(3)]);

    % compose C
    if i ==1
        C.cores{i,1} = [A.cores{i,1}, -B.cores{i,1}];
        C.cores{i,1} = reshape(C.cores{i,1},[a(1),a(2),a(3)+b(3)]);
        C.dim(i,:) = [a(1),a(2),a(3)+b(3)];
    elseif i == d
        C.cores{i,1} = [A.cores{i,1}; B.cores{i,1}];
        C.cores{i,1} = reshape(C.cores{i,1},[a(1)+b(1),a(2),a(3)]);
        C.dim(i,:) = [a(1)+b(1),a(2),a(3)];
    else
        C.cores{i,1} = [A.cores{i,1}, zeros(a(1),b(2)*b(3));
                    zeros(b(1),a(2)*a(3)), B.cores{i,1}];
        C.cores{i,1} = reshape(C.cores{i,1},[a(1)+b(1),a(2),a(3)+b(3)]);
        C.dim(i,:) = [a(1)+b(1),a(2),a(3)+b(3)];
    end
end
